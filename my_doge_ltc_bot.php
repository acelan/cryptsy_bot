...
class my_doge_bot extends cryptsy_bot
{
	public function my_doge_bot($config)
	{
		...
		// It's a bot to sell/buy DOGE and LTC only
		$this->set_key($config['public_key'] , $config['private_key'], "DOGE/LTC");
	}
	...
	protected function tick($data)
	{
		...
		else if(sizeof($my_orders) == 1) // bought or sold an order, cancel old order and re-create all orders
		{
			if(sizeof($data["my_trade"]) != 0)
			{
				...
				{
					print($my_trade["tradetype"]." ".$my_trade["quantity"]." at price ".$my_trade["tradeprice"]." , total ltc ".$my_trade["total"]."\n");

					$file = $this->log_filename;
					if($file != "")
						file_put_contents($file, $my_trade["tradetype"]." ".$my_trade["quantity"]." at price ".$my_trade["tradeprice"]." , total ltc ".$my_trade["total"]."\n", FILE_APPEND);
	...
	{
		$my_ltc = $this->data["my_wallet"]["LTC"];
		$my_doge = $this->data["my_wallet"]["DOGE"];
		$cur_buy_price = $this->data["cur_buy_price"];
		$cur_sell_price = $this->data["cur_sell_price"];

		if( $my_ltc > 0)
			$this->create_buy_order($cur_sell_price*pow($this->stop_lost_percent,$this->buy_count+1), floor($my_ltc/$cur_sell_price*$this->order_proportion));
		if( $my_doge > 0)
		{
	...
	protected function show_status()
	{
		...
		$total_ltc = $my_ltc + $myorder_ltc + ($my_doge+$myorder_doge)*$cur_buy_price;
		print("=================================================================================\n");
		print($datetime." - total ltc=".$total_ltc." , cur_price=".$cur_buy_price.", my_ltc=".$my_ltc." , my_doge=".$my_doge." , b_c=".$this->buy_count." , s_c=".$this->sell_count."\n");
		foreach($my_orders as $order)
			print("   my orders - ".$order["ordertype"]." ".$order["quantity"]." at price ".$order["price"]." , total ltc ".$order["total"]."\n");

		if($this->log_filename != "")
		{
			$file = $this->log_filename;
			file_put_contents($file, "=================================================================================\n", FILE_APPEND);
			file_put_contents($file,$datetime." - total ltc=".$total_ltc." , cur_price=".$cur_buy_price.", my_ltc=".$my_ltc." , my_doge=".$my_doge." , b_c=".$this->buy_count." , s_c=".$this->sell_count."\n", FILE_APPEND);
			foreach($my_orders as $order)
				file_put_contents($file, "   my orders - ".$order["ordertype"]." ".$order["quantity"]." at price ".$order["price"]." , total ltc ".$order["total"]."\n", FILE_APPEND);
		}
	}
