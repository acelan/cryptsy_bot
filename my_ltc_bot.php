...
include "config_ltc.php";

class my_ltc_bot extends cryptsy_bot
{
	public function my_ltc_bot($config)
	{
		...
		// It's a bot to sell/buy LTC and BTC only
		$this->set_key($config['public_key'] , $config['private_key'], "LTC/BTC");
	}
	...
	protected function place_order()
	{
		...
		if( $my_btc > 0)
			$this->create_buy_order($cur_sell_price*pow($this->stop_lost_percent,$this->buy_count+1), $my_btc/$cur_sell_price*$this->order_proportion);
		if( $my_ltc > 0)
		{
			// we don't want to sell too much if we just bought more than 1 time
			$quantity = $my_ltc*pow($this->sell_proportion,$this->buy_count+1);
	...
	protected function show_status()
	{
		...
		$total_btc = $my_btc + $myorder_btc + ($my_ltc+$myorder_ltc)*$cur_buy_price;
		print("=================================================================================\n");
		print($datetime." - total btc=".$total_btc." , cur_price=".$cur_buy_price.", my_btc=".$my_btc." , my_ltc=".$my_ltc." , b_c=".$this->buy_count." , s_c=".$this->sell_count."\n");
		foreach($my_orders as $order)
		...
		if($this->log_filename != "")
		{
			...
			file_put_contents($file,$datetime." - total btc=".$total_btc." , cur_price=".$cur_buy_price.", my_btc=".$my_btc." , my_ltc=".$my_ltc." , b_c=".$this->buy_count." , s_c=".$this->sell_count."\n", FILE_APPEND);
			foreach($my_orders as $order)
